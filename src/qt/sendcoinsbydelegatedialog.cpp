    ...
    model(0)
{
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::bitcreditAddressFont());
...
void SendCoinsByDelegateDialog::on_sendButton_clicked()
{
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins By Delegate"),
            ...
            arg(BitcreditUnits::formatWithUnit(BitcreditUnits::BCR, 10000000)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
void SendCoinsByDelegateDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CBitcreditAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsByDelegateDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        CoinControlDialog::coinControl->destChange = CBitcreditAddress(text.toStdString()).Get();

        // label for the change address
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:black;}");
        if (text.isEmpty())
            ui->labelCoinControlChangeLabel->setText("");
        else if (!CBitcreditAddress(text.toStdString()).IsValid())
        {
        ...
        else
        {
            ...
            else
            {
                CPubKey pubkey;
                CKeyID keyid;
                CBitcreditAddress(text.toStdString()).GetKeyID(keyid);   
